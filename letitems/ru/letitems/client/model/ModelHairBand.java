...
public class ModelHairBand extends ModelBiped {
   ...
   private ModelHairBand(int get, int parts, String sub) {
      ...
      IModelCustom model = AdvancedModelLoader.loadModel(new ResourceLocation("letitems:models/hair/" + get + ".obj"));
   ...
   private byte getPlayerColor(EntityPlayer player) {
      ...
      if (hairPlayers.containsKey(uuid)) {
         return (Byte)hairPlayers.get(uuid);
      } else {
         ItemStack hairStack = ExtendedPlayer.getExtendedPlayer(player).getHair();
         if (hairStack != null && hairStack.hasTagCompound()) {
            byte color = hairStack.getTagCompound().getByte("type");
            hairPlayers.put(uuid, color);
            return color;
         } else {
            return 0;
         }
   ...
   private ResourceLocation getModelTexture(String id) {
      if (!this.locationHashMap.containsKey(id)) {
         this.locationHashMap.put(id, new ResourceLocation("letitems", "textures/models/hairs/" + id + ".png"));
      }
